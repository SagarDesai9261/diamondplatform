...
class employee_display{
  Stream<List<EmployeeDetails>> fetchEmployee() async*{
    try{
     ...
     //  print(token);
      var response =await http.get(Uri.parse("https://diamond-platform-12038fd67b59.herokuapp.com/employee/all"),headers: {
      ...
      if(response.statusCode == 200){
        ...
        //  print(data);
        yield data.map((jobPostJson) => EmployeeDetails.fromJson(jobPostJson)).toList();
      } else{
    ...
    while(true) {
      ...
      try {
        var response = await http.get(
          Uri.parse(
              "https://diamond-platform-12038fd67b59.herokuapp.com/findjob/findjob/all"),
        );
        ...
        if (response.statusCode == 200) {
          ...
          final List<dynamic> data = jsonData['data'];
          //  print(data);
          yield data.map((jobPostJson) => employee_job_post.fromJson(jobPostJson))
              .toList();
        } else {
...
class JobPostService {
    ...
    var id = data["_id"];
      while(true) {
        ...
        try {
          var response = await http.get(
            Uri.parse(
                "https://diamond-platform-12038fd67b59.herokuapp.com/jobpost/viewjobpost/$id"),
            headers: {
              'Authorization': 'Bearer $token',
            },
          );
          ...
          if (response.statusCode == 200) {
            ...
            final List<dynamic> data = jsonData['data'];
            //  print(data);
            yield data.map((jobPostJson) => JobPost.fromJson(jobPostJson))
                .toList();
          } else {
            throw Exception('Failed to load job posts');
          }
  ...
  jobPostDelete(String _id) async {
    ...
    try{
    var response = await http.delete(Uri.parse("https://diamond-platform-12038fd67b59.herokuapp.com/jobpost/jobpost/remove/$_id"),
        headers: {
          'Authorization': 'Bearer $token',
        },
    );
...
class BannerDataProvider with ChangeNotifier {
  ...
  Future<void> fetchBanners() async {
    ...
    var response = await http.get(Uri.parse("https://diamond-platform-12038fd67b59.herokuapp.com/banner/banner/$isEmployee"));
...
class employee_job_post_details{
  ...
  //  Map<String, dynamic> data = json.decode(prefs.getString("user_details")!);
    //var id = data["_id"];
      ...
      try {
        var response = await http.get(
          Uri.parse("https://diamond-platform-12038fd67b59.herokuapp.com/viewjobs/viewjobs"),
          headers: {
            'Authorization': 'Bearer $token',
          },
        );
